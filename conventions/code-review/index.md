...
코드 리뷰는 동료가 작성한 코드를 점검하고 피드백을 교환하는 과정입니다. 새로 추가 또는 변경하는 코드에 대해서 구성원 모두가 책임감을 가지고 코드를 검토하는 매우 중요한 활동입니다. 이 문서는 코드 리뷰를 받거나, 코드 리뷰를 하는 사람이 알아야 할 지침을 제공합니다.



# 2. 코드 리뷰 기본 원칙

- 모든 코드는 코드 리뷰를 거쳐야만 머지를 해야 합니다.
...
코드 리뷰는 PR(Pull Request)을 만듦으로써 시작합니다. 충분한 정보를 제공하는 PR은 리뷰어가 더 정확하고 신속한 리뷰를 할 수 있도록 돕습니다. 이는 팀의 생산성을 향상시킵니다.
...
- 동료가 더 적은 시간 동안에, 더 집중해서 리뷰를 할 수 있습니다.
...
- 테스트가 작성되어 있다면 [테스트 가이드](https://wiki.mm.meshkorea.net/pages/viewpage.action?pageId=78939828)를 참고해서 개선할 부분이 있는지를 확인합니다.
